...
if (isset($_POST["username"])) {
	...
	if ($username == NULL | $password == NULL | $email == NULL) {
		...
		$checkemail = mysqli_query($con, "SELECT email FROM dg_usuario WHERE correo='$email'");
		...
		if ($email_exist>0|$username_exist>0) {
			...
			$query = "INSERT INTO dg_usuario (user, pass, nombre, apellido, cargo, condicion, correo)
			VALUES (' $username ', ' $password ', ' $nombre ', ' $nombre ', ' $departamento ', ' $condicion ' ,' $email ')";
